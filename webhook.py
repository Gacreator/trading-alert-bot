...
def _evaluate_system_c(wallet, mint, current_price, current_market_cap, details, result, pair, score, prior_pc_5m):
    if score != 100:
        return
    ...
    trend_label = price_trend_label(pc_5m, prior_pc_5m)
    if "unclear" in trend_label:
        return
    ...
    for (trade_id, wallet, mint, entry_price, peak_price,
        ...
        try:
            ...
            if closed:
                ...
                print(f"📝 SYSTEM C CLOSED: {mint} via {close_reason}")
            else:
                c2.execute(
                    """
                    UPDATE paper_trades_c
...
@app.route("/paper-trades-b")
def paper_trades_b_report():
    since_param = request.args.get("since")
    conn = get_conn()
    try:
        c = conn.cursor()

        query = "SELECT wallet, token_mint, entry_price, entry_time, peak_price, remaining_pct, status, close_reason, closed_at, realized_return_pct FROM paper_trades_b"
        params = []
        if since_param:
            query += " WHERE entry_time >= %s"
            params.append(since_param)
        ...
        if not rows:
            return "No System B paper trades yet.", 200
        ...
        lines = [f"<b>System B Paper Trading Report</b>{range_label}<br>"]
        lines.append(f"<br>Open positions: {len(open_trades)}")
        lines.append(f"Closed positions: {len(closed_trades)}")
        ...
        return f"paper_trades_b_report error: {e}", 500
    ...
    try:
        ...
        c.execute("""
            SELECT wallet, token_mint, first_seen_at, buy_count,
                   ...
                   block_reason_at_last_attempt, token_name, token_symbol,
                   has_logo, has_website, has_social
            FROM wallet_token_history
            WHERE token_mint = %s
        """, (mint,))
        rows = c.fetchall()
        ...
        for wallet, token_mint, first_seen_at, buy_count, price_at_first_buy, pumped_3x, momentum_alerted, pumped_since_rec, last_checked, block_reason, token_name, token_symbol, has_logo, has_website, has_social in rows:
            lines.append(
                ...
                f"token_name: {token_name or 'not captured'}<br>"
                f"token_symbol: {token_symbol or 'not captured'}<br>"
                f"has_logo: {has_logo}<br>"
                f"has_website: {has_website}<br>"
                f"has_social: {has_social}<br>"
            )
